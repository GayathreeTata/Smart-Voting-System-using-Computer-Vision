...
sender_address ="nagamchenchulakshmi@gmail.com" #enter sender's email id
sender_pass = 'fcgedzzfncmuvole' #enter sender's passwor
...
def voting():
    pkl_file = open(r'D:\VAMSI\CSE_PROJECTS\Roshni\SMART VOTING SYSTEM\encoder.pkl', 'rb')
    my_le = pickle.load(pkl_file)
    pkl_file.close()
    recognizer = cv2.face.LBPHFaceRecognizer_create()  # cv2.createLBPHFaceRecognizer()
    recognizer.read(r"D:\VAMSI\CSE_PROJECTS\Roshni\SMART VOTING SYSTEM\Trained.yml")
    cam=cv2.VideoCapture(0, cv2.CAP_DSHOW)
